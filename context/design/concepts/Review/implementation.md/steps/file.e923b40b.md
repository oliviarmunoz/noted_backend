---
timestamp: 'Mon Nov 24 2025 00:05:32 GMT-0500 (Eastern Standard Time)'
parent: '[[../20251124_000532.33f9d33f.md]]'
content_id: e923b40b5122a99d6a3ab1e9ee6c78c6f8a523c888a484501b9edad8e3a52e57
---
...
import { spotifyService } from "@utils/spotify.ts"; // Import Spotify service
...
type User = ID;
type Item = ID; // Item type is generic, will hold Spotify IDs or other external IDs
...
export default class ReviewConcept {
  ...
  /**
   * Query: _getReviewComments (review: Review): Id[]
   *
   * @effects Returns all comment IDs associated with the given review.
   */
  async _getReviewComments({ review }: { review: Review }): Promise<ID[]> {
    const reviewDoc = await this.reviews.findOne({ _id: review }, { projection: { comments: 1 } });
    return reviewDoc?.comments.map(c => c.commentId) || [];
  }

  /**
   * Query: _getSearchForItem (query: String): Item[]
   *
   * @effects Returns all items (Spotify IDs) associated with the search query using Spotify's API.
   *          Aggregates IDs from tracks, albums, and artists.
   */
  async _getSearchForItem({ query }: { query: string }): Promise<Item[]> {
    try {
      const spotifyResults = await spotifyService.searchAll(query);
      const itemIds = new Set<Item>(); // Use a Set to ensure unique IDs

      // Extract IDs from tracks, if available
      spotifyResults.tracks?.items?.forEach((track: { id: string }) => itemIds.add(track.id as Item));
      // Extract IDs from albums, if available
      spotifyResults.albums?.items?.forEach((album: { id: string }) => itemIds.add(album.id as Item));
      // Extract IDs from artists, if available
      spotifyResults.artists?.items?.forEach((artist: { id: string }) => itemIds.add(artist.id as Item));

      return Array.from(itemIds);
    } catch (error) {
      console.error("Error searching Spotify for items:", error);
      // Queries typically return an empty array on failure, rather than an error object.
      return [];
    }
  }
}
