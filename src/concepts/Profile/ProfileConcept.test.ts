...
Deno.test(
  "Profile Concept: Principle fulfillment - User sets profile, others view it",
  async () => {
    ...
    try {
      ...
      assertEquals(
        "error" in bioUpdateResult,
        false,
        "Updating bio should succeed."
      );

      // Trace Step 2: Alice updates her profile picture
      console.log(
        `Action: ${userAlice} updates thumbnail to "https://example.com/alice.jpg"`
      );
      ...
      assertEquals(
        "error" in thumbnailUpdateResult,
        false,
        "Updating thumbnail should succeed."
      );

      // Trace Step 3: Bob (another user) views Alice's profile
      console.log(`Query: ${userBob} gets ${userAlice}'s profile`);
      const aliceProfile = await profileConcept._getProfile({
        user: userAlice,
      });
      ...
      assertEquals(
        aliceProfile.length,
        1,
        "Should return exactly one profile for Alice."
      );
      assertEquals(
      ...
      console.log(
        `Result: Alice's profile bio: '${aliceProfile[0].profile.bio}', thumbnail: '${aliceProfile[0].profile.thumbnailUrl}'`
      );

      console.log(
        "Principle successfully demonstrated: Alice's profile was set and viewed by another user."
      );
    } finally {
      await client.close();
    }
...
Deno.test("Profile Concept: Action - updateBio behavior", async (t) => {
  ...
  await t.step("should create a new profile if one doesn't exist", async () => {
    ...
    assertEquals(
      initialProfile.length,
      0,
      "Bob should not have a profile initially."
    );

    const result = await profileConcept.updateBio({
      user: userBob,
      bio: "Digital Artist",
    });
    assertEquals(
      "error" in result,
      false,
      "updateBio should succeed for a new user."
    );
    ...
    assertEquals(
      updatedProfile[0].profile.bio,
      "Digital Artist",
      "Bob's bio should be 'Digital Artist'."
    );
    assertEquals(
      updatedProfile[0].profile.thumbnailUrl,
      undefined,
      "Bob's thumbnail should be undefined as it wasn't set."
    );
  ...
  await t.step("should update an existing bio", async () => {
    ...
    assertEquals(
      initialBio[0].bio,
      "Initial Bio",
      "Initial bio should be 'Initial Bio'."
    );
    ...
    assertEquals(
      "error" in result,
      false,
      "Updating existing bio should succeed."
    );
    ...
    assertEquals(
      updatedBio[0].bio,
      "Updated Bio",
      "Bio should be updated to 'Updated Bio'."
    );
    console.log(
      `Effect confirmed: ${userBob}'s bio updated from 'Initial Bio' to 'Updated Bio'.`
    );
  });
...
Deno.test("Profile Concept: Action - updateThumbnail behavior", async (t) => {
  ...
  await t.step("should create a new profile if one doesn't exist", async () => {
    console.log(
      `--- Test: updateThumbnail creates new profile for ${userCharlie} ---`
    );
    const initialProfile = await profileConcept._getProfile({
      user: userCharlie,
    });
    assertEquals(
      initialProfile.length,
      0,
      "Charlie should not have a profile initially."
    );

    const result = await profileConcept.updateThumbnail({
      user: userCharlie,
      thumbnailUrl: "https://example.com/charlie.png",
    });
    assertEquals(
      "error" in result,
      false,
      "updateThumbnail should succeed for a new user."
    );

    const updatedProfile = await profileConcept._getProfile({
      user: userCharlie,
    });
    assertEquals(
      updatedProfile.length,
      1,
      "Charlie should now have a profile."
    );
    assertEquals(
      updatedProfile[0].profile.thumbnailUrl,
      "https://example.com/charlie.png",
      "Charlie's thumbnail should be 'https://example.com/charlie.png'."
    );
    assertEquals(
      updatedProfile[0].profile.bio,
      undefined,
      "Charlie's bio should be undefined as it wasn't set."
    );
    console.log(
      `Effect confirmed: ${userCharlie} now has thumbnail 'https://example.com/charlie.png'.`
    );
  });

  await t.step("should update an existing thumbnail", async () => {
    console.log(
      `--- Test: updateThumbnail updates existing thumbnail for ${userCharlie} ---`
    );
    // Ensure Charlie has an initial profile with a thumbnail
    await profileConcept.updateThumbnail({
      user: userCharlie,
      thumbnailUrl: "https://example.com/old_charlie.png",
    });
    const initialThumbnail = await profileConcept._getThumbnail({
      user: userCharlie,
    });
    assertEquals(
      initialThumbnail[0].thumbnailUrl,
      "https://example.com/old_charlie.png",
      "Initial thumbnail should be 'https://example.com/old_charlie.png'."
    );

    const result = await profileConcept.updateThumbnail({
      user: userCharlie,
      thumbnailUrl: "https://example.com/new_charlie.png",
    });
    assertEquals(
      "error" in result,
      false,
      "Updating existing thumbnail should succeed."
    );

    const updatedThumbnail = await profileConcept._getThumbnail({
      user: userCharlie,
    });
    assertEquals(
      updatedThumbnail[0].thumbnailUrl,
      "https://example.com/new_charlie.png",
      "Thumbnail should be updated to 'https://example.com/new_charlie.png'."
    );
    console.log(
      `Effect confirmed: ${userCharlie}'s thumbnail updated from 'old' to 'new'.`
    );
  });
...
Deno.test(
  "Profile Concept: Query behavior (_getBio, _getThumbnail, _getProfile)",
  async (t) => {
    const [db, client] = await testDb();
    const profileConcept = new ProfileConcept(db);

    await t.step(
      "should return empty arrays for non-existent users",
      async () => {
        ...
        assertEquals(
          bio.length,
          0,
          "Bio query for non-existent user should be empty."
        );
        console.log(
          `Effect confirmed: _getBio returns empty for ${userAlice}.`
        );

        const thumbnail = await profileConcept._getThumbnail({
          user: userAlice,
        });
        assertEquals(
          thumbnail.length,
          0,
          "Thumbnail query for non-existent user should be empty."
        );
        console.log(
          `Effect confirmed: _getThumbnail returns empty for ${userAlice}.`
        );
        ...
        assertEquals(
          profile.length,
          0,
          "Profile query for non-existent user should be empty."
        );
        console.log(
          `Effect confirmed: _getProfile returns empty for ${userAlice}.`
        );
      }
    );
    ...
    await t.step("should retrieve correct bio", async () => {
      ...
      console.log(
        `Effect confirmed: _getBio returns 'Explorer' for ${userBob}.`
      );
    });
    ...
    await t.step("should retrieve correct thumbnail", async () => {
      ...
      await profileConcept.updateThumbnail({
        user: userBob,
        thumbnailUrl: "http://explorer.com/pic.jpg",
      });
      ...
      assertEquals(
        ...
        "http://explorer.com/pic.jpg",
        "Thumbnail should be 'http://explorer.com/pic.jpg'."
      );
      console.log(
        `Effect confirmed: _getThumbnail returns 'http://explorer.com/pic.jpg' for ${userBob}.`
      );
    });

    await t.step(
      "should retrieve partial profile if only one field is set",
      async () => {
        console.log(
          `--- Test: Retrieve partial profile for ${userCharlie} (only bio) ---`
        );
        ...
        assertEquals(
          profile[0].profile.bio,
          "Only Bio",
          "Bio should be 'Only Bio'."
        );
        assertEquals(
          profile[0].profile.thumbnailUrl,
          undefined,
          "Thumbnail should be undefined."
        );
        console.log(
          `Effect confirmed: _getProfile for ${userCharlie} returns bio and undefined thumbnail.`
        );
      }
    );
...
Deno.test("Profile Concept: Mixed updates and queries", async () => {
  ...
  try {
    ...
    assertEquals(
      profile[0].profile.thumbnailUrl,
      undefined,
      "Thumbnail should still be undefined."
    );
    ...
    await profileConcept.updateThumbnail({
      user: userAlice,
      thumbnailUrl: "http://writer.com/pic.png",
    });
    profile = await profileConcept._getProfile({ user: userAlice });
    assertEquals(
      profile[0].profile.bio,
      "Writer",
      "Bio should still be 'Writer'."
    );
    assertEquals(
      profile[0].profile.thumbnailUrl,
      "http://writer.com/pic.png",
